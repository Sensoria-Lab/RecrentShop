...
import { filterReducer, initialFilterState, hasPendingFilterChanges, type FilterAction } from '@/src/types/filters';

type PendingFilterActionType =
...
export const useCatalogFilters = () => {
  ...
  const toggleFilterValue = useCallback(<T extends string>(
    value: T,
    currentValues: T[],
    actionType: PendingFilterActionType
  ) => {
    ...
    switch (actionType) {
      ...
      case 'SET_PENDING_COLLECTION_FILTER':
        dispatch({ type: actionType, payload: newValues } as FilterAction);
        break;
      default:
        break;
    }
