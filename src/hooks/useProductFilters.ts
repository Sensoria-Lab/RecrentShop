...
export const useProductFilters = (products: Product[], filters: FilterOptions) => {
  const filteredProducts = useMemo(() => {
    ...
    if (filters.colorFilter.length > 0) {
      filtered = filtered.filter(p => isColorFilterValue(p.color) && filters.colorFilter.includes(p.color));
    }
    ...
    if (filters.clothingTypeFilter.length > 0) {
      filtered = filtered.filter(p => {
        ...
        return filters.clothingTypeFilter.includes(p.clothingType);
      });
    }
    ...
    if (filters.collectionFilter.length > 0) {
      filtered = filtered.filter(p => {
        ...
        return filters.collectionFilter.includes(p.collection);
      });
    }
