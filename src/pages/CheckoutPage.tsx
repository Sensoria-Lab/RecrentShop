...
const CheckoutPage: React.FC = () => {
  ...
  return (
    ...
    <div 
      className="space-y-3 sm:space-y-4 mb-4 sm:mb-5 md:mb-6 max-h-[200px] sm:max-h-[250px] md:max-h-[300px] overflow-y-auto pr-2 custom-scrollbar"
    >
      {cart.map((item) => (
        <div key={item.id} className="flex gap-0 p-3 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-colors min-h-[80px]">
          <Img
            src={item.image}
            alt={item.title}
            className="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-lg border border-white/10 flex-shrink-0"
          />
          <div className="flex-1 flex flex-col ml-2 sm:ml-3">
            <div className="flex-1">
              <h3 className="text-xs sm:text-sm md:text-base font-semibold text-white truncate">
                {item.title}
              </h3>
              <p className="text-[10px] sm:text-xs text-gray-400 mt-1 line-clamp-2 overflow-hidden">
                {item.subtitle}
    ...
    </div>                <div className="border-t border-white/20 pt-5 sm:pt-6 mt-5 sm:mt-6 space-y-3">
                  {step === 'delivery' && (
